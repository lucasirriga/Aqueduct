...
class DimensionarRedeSetorizadaDialog(QDialog):
    def __init__(self, parent=None):
        ...
        f_hid.addRow("HF Máx Principal (Fonte-Válv):", self.spin_hf_prin)

        self.spin_hf_deriv = QDoubleSpinBox()
        self.spin_hf_deriv.setRange(0.1, 500.0)
        self.spin_hf_deriv.setValue(5.0)
        self.spin_hf_deriv.setSuffix(" mca")
        f_hid.addRow("HF Máx Derivação (Válv-Lat):", self.spin_hf_deriv)
        ...
        self.spin_seg.setRange(1.0, 100.0)
        self.spin_seg.setValue(6.0)
        self.spin_seg.setSuffix(" m")
        f_hid.addRow("Segmentação Comercial (m):", self.spin_seg)
    ...
    def _salvar_config(self):
        s = QgsSettings()
        s.setValue("Aqueduct/Setorizada/ps", self.spin_ps.value())
        s.setValue("Aqueduct/Setorizada/diams_lat", self.edit_diams_lat.text())
        s.setValue("Aqueduct/Setorizada/diams_prin", self.edit_diams_prin.text())
        s.setValue("Aqueduct/Setorizada/hf_prin", self.spin_hf_prin.value())
        s.setValue("Aqueduct/Setorizada/hf_deriv", self.spin_hf_deriv.value())
        s.setValue("Aqueduct/Setorizada/vel_max", self.spin_vel_max.value())
        s.setValue("Aqueduct/Setorizada/tol", self.spin_tol.value())
        s.setValue("Aqueduct/Setorizada/vazao", self.spin_vazao.value())
        s.setValue("Aqueduct/Setorizada/simult", self.spin_simult.value())
        s.setValue("Aqueduct/Setorizada/seg", self.spin_seg.value())

    def _carregar_config(self):
        s = QgsSettings()
        self.spin_ps.setValue(s.value("Aqueduct/Setorizada/ps", 30.0, type=float))
        self.edit_diams_lat.setText(s.value("Aqueduct/Setorizada/diams_lat", "16, 20, 25, 32"))
        self.edit_diams_prin.setText(s.value("Aqueduct/Setorizada/diams_prin", "50, 75, 100, 150"))
        self.spin_hf_prin.setValue(s.value("Aqueduct/Setorizada/hf_prin", 10.0, type=float))
        self.spin_hf_deriv.setValue(s.value("Aqueduct/Setorizada/hf_deriv", 5.0, type=float))
        self.spin_vel_max.setValue(s.value("Aqueduct/Setorizada/vel_max", 2.0, type=float))
        self.spin_tol.setValue(s.value("Aqueduct/Setorizada/tol", 0.10, type=float))
        self.spin_vazao.setValue(s.value("Aqueduct/Setorizada/vazao", 50.0, type=float))
        self.spin_simult.setValue(s.value("Aqueduct/Setorizada/simult", 1, type=int))
        self.spin_seg.setValue(s.value("Aqueduct/Setorizada/seg", 6.0, type=float))
    ...
    def get_inputs(self):
        return {
            ...
            'hf_deriv': self.spin_hf_deriv.value(),
            'vel_max': self.spin_vel_max.value(),
            'seg_len': self.spin_seg.value(),
        }
...
class DimensionarRedeSetorizadaTool(AqueductTool):
    ...
    def _executar(self, inp, log):
        ...
        lyr_planar = res['OUTPUT']
        
        # B. Segmentação por Comprimento Comercial
        seg_len = inp['seg_len']
        log(f"✂️ Segmentando tubos longos em trechos de {seg_len}m...")
        res_seg = processing.run("native:splitlinesbylength", {
            'INPUT': lyr_planar,
            'LENGTH': seg_len,
            'OUTPUT': 'memory:'
        })
        lyr_split = res_seg['OUTPUT']
        ...
        # Otimizar HF do caminho Principal (Fonte -> Válvulas)
        ...
        while not concluido_prin:
            ...
            def find_worst_main_path(curr, current_hf, path):
                ...
                # Se chegou numa válvula, o caminho principal termina aqui
                if curr in node_valves:
                    if current_hf > max_path_hf:
                        max_path_hf = current_hf
                        worst_path_nodes = list(path)
                    return
                
                has_children_prin = False
                for v, eid in children.get(curr, []):
                    if edges[eid]['tipologia'] == 'Principal':
                        has_children_prin = True
                        edges[eid]['HF'] = calcular_hf_hw(edges[eid]['V'], edges[eid]['DN'], edges[eid]['L'])
                        find_worst_main_path(v, current_hf + edges[eid]['HF'], path + [eid])
                
                if not has_children_prin:
                    if current_hf > max_path_hf:
                        max_path_hf = current_hf
                        worst_path_nodes = list(path)
            
            find_worst_main_path(root_node, 0.0, [])
                    
            if max_path_hf <= inp['hf_prin']:
                concluido_prin = True
            else:
                aumentou = False
        ...
        # Otimizar HF do caminho Derivação (Válvulas -> Laterais)
        for v_node in node_valves:
            concluido_deriv = False
            while not concluido_deriv:
                max_path_hf = 0.0
                worst_path_nodes = []
                
                def find_worst_deriv_path(curr, current_hf, path):
                    nonlocal max_path_hf, worst_path_nodes
                    has_children_deriv = False
                    for v, eid in children.get(curr, []):
                        if edges[eid]['tipologia'] == 'Derivacao':
                            has_children_deriv = True
                            edges[eid]['HF'] = calcular_hf_hw(edges[eid]['V'], edges[eid]['DN'], edges[eid]['L'])
                            find_worst_deriv_path(v, current_hf + edges[eid]['HF'], path + [eid])
                    
                    if not has_children_deriv:
                        if current_hf > max_path_hf:
                            max_path_hf = current_hf
                            worst_path_nodes = list(path)
                
                find_worst_deriv_path(v_node, 0.0, [])
                
                if max_path_hf <= inp['hf_deriv']:
                    concluido_deriv = True
                else:
                    aumentou = False
                    for eid in worst_path_nodes:
                        curr_dn = edges[eid]['DN']
                    ...
                    if not aumentou:
                        concluido_deriv = True
                        log(f"   ⚠️ Derivação após válvula {v_node} estourou {inp['hf_deriv']}mca (Chegou a {max_path_hf:.2f}mca).")
        ...
        # 8. Fusão de Trechos com mesmo Diâmetro e Adjacentes (Cleanup)
        log("🧹 Unindo trechos adjacentes com mesmo diâmetro...")
        
        def merge_geoms(g1, g2, shared_node_pt):
            pl1 = g1.asPolyline()
            pl2 = g2.asPolyline()
            # Alinhar extremidades
            if pl1[-1].distance(shared_node_pt) > pl1[0].distance(shared_node_pt): pl1.reverse()
            if pl2[0].distance(shared_node_pt) > pl2[-1].distance(shared_node_pt): pl2.reverse()
            return QgsGeometry.fromPolylineXY(pl1 + pl2[1:])

        # Nós especiais que impedem a união
        nós_bloqueio = set(node_valves)
        nós_bloqueio.add(root_node)
        # Adicionar nós de emissores
        for ep in em_pts.values():
            nearest = node_idx.nearestNeighbor(ep['geom'].asPoint(), 1)
            if nearest: nós_bloqueio.add(nearest[0])

        merged_any = True
        while merged_any:
            merged_any = False
            # Recalcular adjacência atualizada
            curr_adj = {}
            for eid, e in edges.items():
                curr_adj.setdefault(e['n1'], []).append(eid)
                curr_adj.setdefault(e['n2'], []).append(eid)
            
            for nid, e_ids in curr_adj.items():
                if nid in nós_bloqueio: continue
                if len(e_ids) == 2:
                    id1, id2 = e_ids
                    e1, e2 = edges[id1], edges[id2]
                    # Mesma tipologia e mesmo DN?
                    if e1['tipologia'] == e2['tipologia'] and e1['DN'] == e2['DN']:
                        # Unir!
                        pt_shared = nodes[nid]
                        new_geom = merge_geoms(e1['geom'], e2['geom'], pt_shared)
                        new_n1 = e1['n2'] if e1['n1'] == nid else e1['n1']
                        new_n2 = e2['n2'] if e2['n2'] == nid else e2['n2']
                        
                        new_eid = f"merged_{id1}_{id2}"
                        edges[new_eid] = {
                            'geom': new_geom, 'L': e1['L'] + e2['L'],
                            'n1': new_n1, 'n2': new_n2,
                            'tipologia': e1['tipologia'], 'DN': e1['DN'],
                            'V': e1['V'], 'HF': e1['HF'] + e2['HF'], # V é aproximado (montante)
                            'f_orig': e1['f_orig']
                        }
                        del edges[id1]
                        del edges[id2]
